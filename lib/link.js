...
const debug = require('debug')('TuyaLink');
...
*  console.log('Done!');
* });
* @returns {Promise<>} A Promise that resolves when all data has been transmitted
*/
...
/**
* Sends start pattern of packets
* (1, 3, 6, 10) 144 times with
* a delay between transmits.
* @returns {Promise<>} A Promise that resolves when data has been transmitted
*/
...
TuyaLink.prototype.smartLinkEncode = function (options) {
  ...
  const stringLengthCRC = this._tuyaCRC8([stringLength], 1);
  ...
  for (let x = 0; x < rawDataLengthRoundedUp; x += 4) {
    ...
    // Calculate the CRC
    const crc = this._tuyaCRC8(crcData, 5);
...
TuyaLink.prototype._tuyaCRC8 = function (p, len) {
  let crc = 0;
  let i = 0;
